...
export async function generateMetadata({
  ...
  if (!cafe) {
    return {
      ...
      description: "This cafe could not be found on Aligarh Cafes.",
    }
  }

  const features = []
  if (cafe.couple_friendly) features.push("couple friendly")
  if (cafe.has_wifi) features.push("free WiFi")
  if (cafe.has_ac) features.push("AC")
  if (cafe.has_rooftop) features.push("rooftop seating")
  if (cafe.has_live_music) features.push("live music")
  if (cafe.has_private_cabin) features.push("private cabins")
...
const description = `${cafe.tagline || cafe.name} in Aligarh. ${featureText} View menu, photos and location on Aligarh Cafes.`
  ...
  return {
    ...
    keywords: [
      ...
      `${cafe.name} Aligarh`,
      `${cafe.name} menu`,
      `cafe in ${cafe.areas?.name || "Aligarh"}`,
      "cafes in Aligarh",
      ...(cafe.vibe_tags || []),
    ],
    openGraph: {
